...
namespace Start.Services.Oauth2;

static class Oauth2X {
    public static (string, string) Authorize(AuthorizeController.Req req) {
        if (req.scope == "throw") return ("Error Header", "Error Body");
        return default!;
    }
}
...
public class AuthorizeController : ControllerBase {
    ...
    public IActionResult Get([FromQuery] Req req) {
        ...
        var (error, errorDesc) = Oauth2X.Authorize(req);
